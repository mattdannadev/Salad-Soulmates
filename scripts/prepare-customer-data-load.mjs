...
function readTab(dir, name) {
  ...
  const rows = parse(readFileSync(path, 'utf8'), { bom: true, columns: true, skip_empty_lines: true, trim: true });
...
function issue(issues, tab, row, message) { issues.push({ tab, row, message }); }
function validate(tab, rows, issues) {
  ...
  rows.forEach((row, index) => {
    ...
    required.forEach((key) => { if (!row[key]) issue(issues, tab, rowNumber, `${key} is required.`); });
    if (['Suppliers', 'Supplier_Packs', 'Customer_Pricing'].includes(tab)) ['active'].forEach((key) => { if (row[key] && !bool.test(row[key])) issue(issues, tab, rowNumber, `${key} must be TRUE or FALSE.`); });
    ...
    if (tab === 'Packaging_Profiles') {
      ...
      ['bag_size_gallons', 'bags_per_case', 'label_width_inches', 'label_height_inches'].forEach((key) => { if (!positive(row[key])) issue(issues, tab, rowNumber, `${key} must be greater than zero.`); });
    }
    if (['Opening_Inventory', 'Receipts', 'Customer_Orders'].includes(tab)) {
      const key = tab === 'Customer_Orders' ? 'needed_on' : tab === 'Opening_Inventory' ? 'as_of_date' : 'received_on';
    ...
    const key = tab === 'Suppliers' ? row.supplier_name.toLowerCase() : tab === 'Customers' ? row.customer_name.toLowerCase() : '';
...
async function loadIntoSupabase(tabs) {
  ...
  const { data: ingredients, error: ingredientError } = await db.from('ingredients').select('id,name,default_uom').eq('active', true); fail(ingredientError);
  const { data: products, error: productError } = await db.from('products').select('id,name').eq('active', true); fail(productError);
  const byName = (rows, label) => new Map((rows ?? []).map((row) => [row.name.trim().toLocaleLowerCase('en-US'), row]));
  const ingredientByName = byName(ingredients, 'ingredient');
  const productByName = byName(products, 'product');
  const lookup = (map, name, kind) => { const found = map.get(name.trim().toLocaleLowerCase('en-US')); if (!found) throw new Error(`${kind} not found or inactive: ${name}`); return found; };
  for (const row of tabs.Suppliers) {
    const { error } = await db.from('suppliers').insert({ name: row.supplier_name, contact_name: row.contact_name, email: row.email, phone: row.phone, lead_time_days: row.lead_time_days ? Number(row.lead_time_days) : null, active: row.active.toLowerCase() !== 'false' }); fail(error);
  }
  const { data: suppliers, error: supplierError } = await db.from('suppliers').select('id,name').eq('active', true); fail(supplierError);
  const supplierByName = byName(suppliers, 'supplier');
  for (const row of tabs.Supplier_Packs) {
    const { error } = await db.from('supplier_items').insert({ supplier_id: lookup(supplierByName, row.supplier_name, 'Supplier').id, ingredient_id: lookup(ingredientByName, row.ingredient_name, 'Ingredient').id, supplier_sku: row.supplier_sku, purchase_uom: row.purchase_uom, pack_quantity: Number(row.pack_quantity), pack_quantity_uom: row.pack_quantity_uom, is_preferred: row.is_preferred.toLowerCase() === 'true', active: row.active.toLowerCase() !== 'false', notes: row.notes }); fail(error);
  }
  for (const row of tabs.Customers) {
    const { error } = await db.rpc('save_customer_master', { payload: { id: randomUUID(), revision: 0, name: row.customer_name, contact_name: row.contact_name, email: row.email, phone: row.phone, address: row.address, notes: row.notes } }); fail(error);
  }
  const { data: customers, error: customerError } = await db.from('customers').select('id,name,revision'); fail(customerError);
  const customerByName = byName(customers, 'customer');
  for (const row of tabs.Customer_Pricing) {
    const customer = lookup(customerByName, row.customer_name, 'Customer');
    const { error } = await db.rpc('save_customer_product_option', { payload: { id: randomUUID(), revision: 0, customer_name: customer.name, product_id: lookup(productByName, row.product_name, 'Product').id, label: row.label, packaging_mode: row.packaging_mode, unit_name: row.unit_name, gallons_per_unit: Number(row.gallons_per_unit), unit_price: Number(row.unit_price_usd), currency: 'USD', active: row.active.toLowerCase() !== 'false', is_preferred: row.is_preferred.toLowerCase() === 'true' } }); fail(error);
  }
  for (const row of tabs.Packaging_Profiles) {
    const { error } = await db.rpc('save_packaging_profile', { payload: { id: randomUUID(), expected_version: 0, product_id: lookup(productByName, row.product_name, 'Product').id, status: row.status, bag_size_gallons: Number(row.bag_size_gallons), bags_per_case: Number(row.bags_per_case), label_width_inches: Number(row.label_width_inches), label_height_inches: Number(row.label_height_inches), display_name: row.display_name, ingredient_statement: row.ingredient_statement } }); fail(error);
  }
  for (const row of tabs.Opening_Inventory) {
    ...
    const { error } = await db.from('inventory_events').insert({ ingredient_id: ingredient.id, event_type: 'OpeningBalance', quantity_delta: Number(row.quantity), uom: row.uom, reason_note: row.reason_note, request_id: randomUUID(), created_at: `${row.as_of_date}T12:00:00Z` }); fail(error);
  }
  for (const row of tabs.Receipts) {
    const ingredient = lookup(ingredientByName, row.ingredient_name, 'Ingredient');
    const { error } = await db.rpc('post_inventory_receipt', { payload: { supplier_id: lookup(supplierByName, row.supplier_name, 'Supplier').id, ingredient_id: ingredient.id, received_on: row.received_on, supplier_reference: row.supplier_reference, quantity: Number(row.quantity), uom: row.uom, supplier_lot: row.supplier_lot, expiration_date: row.expiration_date || '', note: row.note, request_id: randomUUID() } }); fail(error);
  }
  const groupedOrders = new Map();
  for (const row of tabs.Customer_Orders) {
    const key = `${row.customer_name}|${row.reference}|${row.needed_on}`;
    const group = groupedOrders.get(key) ?? { id: randomUUID(), customer_name: row.customer_name, reference: row.reference, needed_on: row.needed_on, products: [] };
    group.products.push({ product_id: lookup(productByName, row.product_name, 'Product').id, customer_product_option_id: null, batch_count: Number(row.batch_count) });
    groupedOrders.set(key, group);
  }
  for (const order of groupedOrders.values()) { const { error } = await db.rpc('save_customer_order', { payload: order }); fail(error); }
...
const plan = { generated_at: new Date().toISOString(), source, master: Object.fromEntries(masterTabs.map((tab) => [tab, tabs[tab]])), transactional: Object.fromEntries(transactionalTabs.map((tab) => [tab, tabs[tab]])), counts: Object.fromEntries(Object.entries(tabs).map(([tab, rows]) => [tab, rows.length])), issues };
writeFileSync(resolve(output), `${JSON.stringify(plan, null, 2)}\n`);
if (issues.length) { console.error(`Validation failed with ${issues.length} issue(s). See ${resolve(output)}.`); process.exitCode = 1; } else if (load) { await loadIntoSupabase(tabs); console.log(`Loaded validated customer data: ${resolve(output)}`); } else console.log(`Prepared validated load plan: ${resolve(output)}`);
