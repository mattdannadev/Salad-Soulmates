...
export default async function SerializedInventory({
  db,
  locale = 'en',
}: {
    ...
    <section className="panel">
      ...
      <form action="/receiving/packages" className="serial-search">
        <label htmlFor="package-search">
          {es
            ? 'Escanea o escribe un código, lote de origen o ingrediente'
      ...
      <p>
        {es
          ? 'Últimos paquetes (máximo 200). Cada paquete conserva su lote y saldo.'
          : 'Latest packages (up to 200). Open a package for its source lot, origin, balance, labels, and history.'}
      </p>
      {units.length === 0 ? (
        <p className="empty">
          {es ? 'Aún no hay paquetes identificados.' : 'No serialized packages yet.'}
        </p>
      ) : (
