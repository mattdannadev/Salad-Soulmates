...
import {
  AlertTriangle, Check, ChevronRight, Pencil,
} from 'lucide-react';
...
function IngredientEntry({
  line, preparation, locale, onSaved,
}: {
  line: WorkerLine;
  ...
  const correctionSaveLabel = es ? 'Guardar cambio' : 'Save change';
  const usageSaveLabel = es ? 'Guardar y seguir' : 'Save and continue';
  const issueSaveLabel = es ? 'Guardar nota' : 'Save note';
  const issueNames = { Bucket: es ? 'Cubeta' : 'Bucket', Spice: es ? 'Especia' : 'Spice' };
  ...
  return (
    <div className="worker-ingredient-entry">
      <div className="worker-ingredient-heading">
        <div>
          <p className="worker-kicker">{es ? 'Ingrediente actual' : 'Current ingredient'}</p>
          <h4>{line.ingredient_name}</h4>
        </div>
        {left === 0 && (
        <span className="worker-done">
          <Check size={16} />
          {es ? 'Listo' : 'Done'}
        </span>
        )}
      ...
      <div className="worker-quantity-line">
        <span>
          ...
          {' '}
          <strong>
            {formatNumber(line.required_quantity)}
            {' '}
            {line.uom}
          </strong>
        </span>
            ...
            <div key={usage.id} className="worker-record">
              <div className="worker-record-summary">
                ...
                <button
                  ...
                  disabled={pending}
                  aria-expanded={editingUsage === usage.id}
                  onClick={() => {
              ...
              {canEdit && editingUsage === usage.id && (
              <form
                className="worker-inline-form"
                onSubmit={(event) => {
                  event.preventDefault();
                  startTransition(async () => {
                    const key = `${usage.id}:${correctionQuantity}:${correctionReason.trim()}`;
                    const id = correctionRequest.current?.key === key
                      ? correctionRequest.current.id : crypto.randomUUID();
                    correctionRequest.current = { key, id };
                    const result = await saveBatchWorksheet('correct', {
                      id,
                      usage_id: usage.id,
                      restored_quantity: Number(correctionQuantity),
              ...
              >
                ...
                <label>
                  {`${es ? 'Cantidad a devolver' : 'Amount to return'} (${line.uom})`}
                  <input
                    type="number"
                    min="0.0001"
                    max={usage.quantity}
                ...
                <label>
                  ...
                  <input
                    ...
                    onChange={(event) => setCorrectionReason(event.currentTarget.value)}
                    placeholder={es ? 'Ej. Lote equivocado' : 'e.g. Wrong lot'}
                  />
                </label>
                <div className="worker-inline-actions">
                  ...
                  <button
                    type="submit"
                    disabled={pending || Number(correctionQuantity) <= 0
                    || Number(correctionQuantity) > usage.quantity
                    || correctionReason.trim().length < 3}
                  >
                    {pending ? savingLabel : correctionSaveLabel}
                  </button>
                </div>
              </form>
              )}
      ...
      {left > 0 && canEdit && line.id && (
        <form onSubmit={(event) => {
          ...
          startTransition(async () => {
            ...
            const requestId = request.current?.key === key
        ...
        >
          ...
          <label>
            ...
            <input
              type="number"
              min="0.0001"
              max={Math.min(left, selected?.remaining_quantity ?? left)}
              step="0.0001"
              required
              value={quantity}
          ...
          <button
            type="submit"
            disabled={pending || !selected || Number(quantity) <= 0
            || Number(quantity) > left || Number(quantity) > selected.remaining_quantity}
          >
            {pending ? savingLabel : usageSaveLabel}
      ...
      {issues.length > 0 && (
      <div className="worker-issues" aria-label={es ? 'Problemas anotados' : 'Reported problems'}>
        {issues.map((issue) => (
          <p key={issue.id}>
            <AlertTriangle size={15} aria-hidden="true" />
            <span>
              <strong>{issueNames[issue.category]}</strong>
              {' '}
              ·
              {' '}
              {issue.note}
      ...
      <div className="worker-report">
        ...
        <form
          ...
          onSubmit={(event) => {
            ...
            startTransition(async () => {
              ...
              const id = issueRequest.current?.key === key
                ? issueRequest.current.id : crypto.randomUUID();
        ...
        >
          <fieldset className="worker-issue-type">
            <legend>{es ? '¿Dónde está el problema?' : 'What has a problem?'}</legend>
            <label>
              <input
                type="radio"
                name={`issue-${line.id}`}
                checked={issueCategory === 'Spice'}
                onChange={() => setIssueCategory('Spice')}
              />
              {es ? 'Especia' : 'Spice'}
            </label>
            <label>
              <input
                type="radio"
                name={`issue-${line.id}`}
                checked={issueCategory === 'Bucket'}
                onChange={() => setIssueCategory('Bucket')}
              />
              {es ? 'Cubeta' : 'Bucket'}
            </label>
          </fieldset>
          {issueCategory === 'Bucket' && (
          <label>
            {es ? 'Cubeta / lote' : 'Bucket / lot'}
            <select
              required
              value={issuePackageId}
              onChange={(event) => setIssuePackageId(event.currentTarget.value)}
            >
          ...
          <label>
            ...
            <textarea
              ...
              placeholder={es ? 'Nota breve para el equipo' : 'A short note for the team'}
            />
          </label>
          <div className="worker-inline-actions">
            ...
            <button type="submit" disabled={pending || issueNote.trim().length < 3 || (issueCategory === 'Bucket' && !issuePackageId)}>
              {pending ? savingLabel : issueSaveLabel}
            </button>
          </div>
        </form>
        )}
...
function PreparationCard({ preparation, locale }: { preparation: WorkerPreparation; locale: 'en' | 'es' }) {
  const es = locale === 'es';
  const statusLabels = {
    Complete: es ? 'Lista' : 'Complete',
    Open: es ? 'En preparación' : 'In preparation',
    'Not started': es ? 'Pendiente' : 'Not started',
  };
  ...
  return (
    <article className="panel worker-preparation">
      <div className="worker-card-top">
        <div>
          ...
          <p className="worker-card-subtitle">
            ...
            ·
            {' '}
            {formatNumber(preparation.target_gallons)}
            {' '}
            gal
          </p>
        </div>
        <span className="badge">{statusLabels[preparation.status]}</span>
      </div>
      <div className="worker-progress" aria-label={`${completeCount} / ${lines.length}`}>
        <div>
          <span>{`${completeCount} / ${lines.length} ${es ? 'ingredientes' : 'ingredients'}`}</span>
          <span>
            {Math.round((100 * completeCount) / Math.max(lines.length, 1))}
            %
          </span>
        </div>
      ...
      {preparation.status === 'Not started' && (
        ...
        >
          {es ? 'Comenzar preparación' : 'Start preparation'}
        </button>
      )}
      <section className="worker-full-ingredient-list" aria-label={es ? 'Todos los ingredientes' : 'All ingredients'}>
        ...
        {lines.map((line) => (
          preparation.status === 'Not started'
            ? (
              <div className="worker-ingredient-preview" key={`${line.ingredient_id}-${line.sequence}`}>
                <span>{line.ingredient_name}</span>
                <strong>{`${formatNumber(line.required_quantity)} ${line.uom}`}</strong>
              </div>
            )
            : (
              <IngredientEntry
                key={`${line.id ?? line.ingredient_id}-${line.usages.map((usage) => `${usage.id}:${usage.quantity}`).join(',')}`}
                line={line}
      ...
      {preparation.status !== 'Not started' && (
        <>
          {preparation.status === 'Open' && (
            ...
            >
              {es ? 'Completar preparación y descontar inventario' : 'Complete preparation and relieve inventory'}
            </button>
          )}
        </>
      )}
  ...
  return (
    <div>
      {preparations.map((item) => (
        <PreparationCard key={item.planned_mixer_batch_id} preparation={item} locale={locale} />
      ))}
    </div>
  );
}
