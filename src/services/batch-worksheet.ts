...
import { z } from 'zod';
import {
  BATCH_WORKSHEET_MESSAGES,
  worksheetCorrectionInputSchema,
  worksheetIssueInputSchema,
  worksheetUsageInputSchema,
} from '@/domain/batch-worksheet';
...
const inputSchemas = {
  open: z.uuid(),
  use: worksheetUsageInputSchema,
  correct: worksheetCorrectionInputSchema,
  issue: worksheetIssueInputSchema,
  complete: z.uuid(),
};
const successMessages: Record<WorksheetOperation, string> = {
  open: 'Spice preparation progress saved.',
  use: 'Spice preparation progress saved.',
  correct: 'Blend allocation updated.',
  issue: 'Problem noted for this preparation.',
  complete: 'Spice preparation completed and inventory relieved.',
};

/** Authorization and validation for a worker's worksheet change. */
export default async function saveBatchWorksheetService(
  operation: unknown,
  input: unknown,
): Promise<ActionResult> {
  const kind = operationSchema.safeParse(operation);
  if (!kind.success) return { ok: false, message: 'Unknown worksheet action.' };
  const parsed = inputSchemas[kind.data].safeParse(input);
  ...
  try {
    const result = await saveWorksheetRecord(db, kind.data, parsed.data);
    if (result.error) {
      logFailure(`batch_worksheet_${kind.data}`, result.error);
      return {
        ok: false,
        message: BATCH_WORKSHEET_MESSAGES.find((message) => result.error.message.includes(message))
    ...
    if (!id.success) {
      ...
      return {
        ok: false,
        message: 'The worksheet save could not be confirmed. Retry with the same entry.',
      };
    }
    return { ok: true, id: id.data, message: successMessages[kind.data] };
  } catch (error) {
