...
function addSheet(name, headers, note, widths = []) {
  ...
  sheet.getRange('A1').format = { fill: navy, font: { name: 'Aptos Display', size: 16, bold: true, color: '#FFFFFF' }, horizontalAlignment: 'left', verticalAlignment: 'center' };
  ...
  sheet.getRange('A3').format = { fill: pale, font: { ...font, italic: true, color: '#244A5A' }, wrapText: true, verticalAlignment: 'center' };
  ...
  headerRange.format = { fill: teal, font: { ...font, bold: true, color: '#FFFFFF' }, wrapText: true, verticalAlignment: 'center', borders: { preset: 'all', style: 'thin', color: '#B7D2D8' } };
  headerRange.format.rowHeight = 34;
  const entry = sheet.getRangeByIndexes(5, 0, 75, headers.length);
  entry.format = { font, fill: '#FFFFFF', borders: { preset: 'all', style: 'thin', color: '#D9E2F3' }, verticalAlignment: 'center' };
  entry.format.wrapText = true;
  for (let i = 0; i < headers.length; i += 1) sheet.getRangeByIndexes(0, i, 80, 1).format.columnWidth = widths[i] ?? 18;
...
readme.getRange('A1').format = { fill: navy, font: { name: 'Aptos Display', size: 18, bold: true, color: '#FFFFFF' }, verticalAlignment: 'center' };
...
readme.getRange('A3').format = { fill: pale, font: { ...font, bold: true, color: '#244A5A' }, wrapText: true, verticalAlignment: 'center' };
...
readme.getRange('A5:B12').format = { font, borders: { preset: 'all', style: 'thin', color: '#D9E2F3' }, wrapText: true, verticalAlignment: 'center' };
...
for (const sheetName of ['Suppliers', 'Supplier_Packs', 'Customers', 'Customer_Pricing', 'Packaging_Profiles', 'Opening_Inventory', 'Receipts', 'Customer_Orders']) {
  const sheet = workbook.worksheets.getItem(sheetName);
  sheet.tabColor = teal;
}
workbook.recalculate();
await fs.mkdir(outputDir, { recursive: true });
const preview = await workbook.render({ sheetName: 'Read Me', range: 'A1:H12', scale: 1.5, format: 'png' });
